...
type EventListener1 = (...args: any[]) => any;

interface DraggerCssProps {
  touchAction?: string;
  userSelect?: string;
  userDrag?: string;
...
type LayoutOnFinish = (items: Item[], isAborted: boolean) => any;

type LayoutFunctionCallback = (layout: LayoutData) => any;

type LayoutFunctionCancel = (...args: any[]) => any;
...
type SortDataGetter = (item: Item, element: HTMLElement) => any;

type DragStartPredicate = (
  item: Item,
  event:
    | DraggerStartEvent
    | DraggerMoveEvent
...
type DragSortGetter = (
  this: Grid,
  item: Item
) => Grid[] | null | void | undefined;

interface DragSortHeuristics {
  sortInterval?: number;
...
type DragPlaceholderCreateElement = (item: Item) => HTMLElement;

type DragPlaceholderOnCreate = (
  item: Item,
  placeholderElement: HTMLElement
) => any;

type DragPlaceholderOnRemove = (
  item: Item,
  placeholderElement: HTMLElement
) => any;
...
type DragAutoScrollTargets = Array<Window | HTMLElement | DragAutoScrollTarget>;

type DragAutoScrollTargetsGetter = (item: Item) => DragAutoScrollTargets;

type DragAutoScrollOnStart = (
  item: Item,
  scrollElement: Window | HTMLElement,
  scrollDirection: number
) => any;

type DragAutoScrollOnStop = (
  item: Item,
  scrollElement: Window | HTMLElement,
  scrollDirection: number
) => any;

type DragAutoScrollHandle = (
  item: Item,
  itemClientX: number,
  itemClientY: number,
  itemWidth: number,
...
export class Emitter {
  constructor();
  on(event: string, listener: EventListener1): this;
  off(event: string, listener: EventListener1): this;
...
export class Dragger {
  ...
  off<T extends keyof DraggerEvents>(
    event: T,
...
export class Grid {
  ...
  getItems(
    targets?: HTMLElement | number | Item | Array<HTMLElement | number | Item>
  ): Item[];
