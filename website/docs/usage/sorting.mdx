...
Sorting can be managed through the `sort` prop of the MuuriComponent. The sort will be updated every time that the MuuriComponent `re-rendered` with a new value in the prop or an Item is added.
