...
Filtering can be managed through the `filter` prop of the MuuriComponent. The filter will be updated every time that the MuuriComponent `re-rendered` with a new value in the prop or an Item is added.
